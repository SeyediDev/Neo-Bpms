...
/// <summary>
/// BPMN User Tasks need to be rendered on user interfaces like forms clients, portlets, etc. 
/// The Rendering element provides an extensible mechanism for specifying UI renderings for User Tasks (Task UI). 
/// The element is optional. One or more rendering methods can be provided in a Task definition. 
/// A User Task can be deployed on any compliant implementation, irrespective of the fact whether the implementation supports specified rendering methods or not. 
/// The Rendering element is the extension point for renderings. Things like language considerations are opaque for the
...
{
    public Rendering(UserTask parent, string id)
        : base(parent, id)
    {
    }

    public Rendering(GlobalUserTask parent, string id)
        : base(parent, id)
    {
    }
}
