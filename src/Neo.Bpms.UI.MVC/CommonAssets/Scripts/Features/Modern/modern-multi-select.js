...
class ModernMultiSelect {
    ...
    createHTML() {
        ...
        this.container._modernMultiSelectInstance = this;
    }
    ...
    open() {
        ...
        // Close all other multi-select dropdowns first
        document.querySelectorAll('.modern-multi-select.open').forEach(ms => {
            if (ms !== this.container) {
                ms.classList.remove('open');
                // Find the ModernMultiSelect instance and close it
                const instance = ms._modernMultiSelectInstance;
        ...
        // Fix positioning for dropdown in tooltip
        if (this.container.closest('.filter-tooltip-panel')) {
