...
public class CP_FTP(string hostIP, string userName, string password)
{
    ...
    public void download(string remoteFile, string localFile)
    {
        ...
        ftpRequest = (FtpWebRequest)WebRequest.Create(hostIP + "/" + remoteFile);
    ...
    public void delete(string deleteFile)
    {
        ...
        //{
        /* Create an FTP Request */
        ftpRequest = WebRequest.Create(hostIP + "/" + deleteFile) as FtpWebRequest;
        /* Log in to the FTP Server with the User Name and Password Provided */
        ftpRequest.Credentials = new NetworkCredential(userName, password);
        /* When in doubt, use these options */
        ftpRequest.UseBinary = true;
    ...
    public string getFileCreatedDateTime(string fileName)
    {
        ...
        //{
        /* Create an FTP Request */
        ftpRequest = (FtpWebRequest)WebRequest.Create(hostIP + "/" + fileName);
        ftpRequest.Timeout = -1;
        /* Log in to the FTP Server with the User Name and Password Provided */
        ftpRequest.Credentials = new NetworkCredential(userName, password);
        /* When in doubt, use these options */
        ...
        ftpStream = ftpResponse.GetResponseStream();
        /* Get the FTP Server's Response Stream */
        StreamReader ftpReader = new(ftpStream);
    ...
    public string getFileSize(string fileName)
    {
        ...
        /* Read the Full Response Stream */
        Exception ex1 = null;
        try { if (!ftpReader.EndOfStream) { fileInfo = ftpReader.ReadToEnd(); } }
        catch (Exception ex)
        {
            //Console.WriteLine(ex.ToString()); 
            ex1 = ex;
    ...
    public List<string> directoryListSimple(string directory)
    {
        ...
        //{
        /* Create an FTP Request */
        ftpRequest = (FtpWebRequest)WebRequest.Create(hostIP + "/" + directory);
        ftpRequest.Timeout = -1;
        /* Log in to the FTP Server with the User Name and Password Provided */
        ftpRequest.Credentials = new NetworkCredential(userName, password);
        /* When in doubt, use these options */
        ...
        StreamReader ftpReader = new(ftpStream);
        /* Store the Raw Response */
        var directoryRaw = new Dictionary<string, string>();
        ...
        try
        {
            while (!ftpReader.EndOfStream)
            {
                ...
                if (!directoryRaw.ContainsKey(line))
                    directoryRaw.Add(line, line);
                else
                    break;
            }
        }
