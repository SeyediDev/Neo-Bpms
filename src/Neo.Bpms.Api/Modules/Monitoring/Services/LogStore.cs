...
public class LogStore : ILogStore
{
    ...
    public IEnumerable<LogEntry> GetRecentLogs(int limit = 100, Models.LogLevel? minLevel = null)
    {
    ...
    public LogTimeline GetTimeline(DateTime from, DateTime to, int intervalSeconds = 60)
    {
        ...
        for (var timestamp = from; timestamp < to; timestamp = timestamp.AddSeconds(intervalSeconds))
        {
            ...
            buckets.Add(new LogTimelineBucket
            {
                ...
                TraceCount = bucketLogs.Count(l => l.Level == Models.LogLevel.Trace),
                DebugCount = bucketLogs.Count(l => l.Level == Models.LogLevel.Debug),
                InfoCount = bucketLogs.Count(l => l.Level == Models.LogLevel.Information),
                WarnCount = bucketLogs.Count(l => l.Level == Models.LogLevel.Warning),
                ErrorCount = bucketLogs.Count(l => l.Level == Models.LogLevel.Error),
                CriticalCount = bucketLogs.Count(l => l.Level == Models.LogLevel.Critical)
            });
        }
