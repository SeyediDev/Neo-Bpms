...
/// The actual Participant from which the Message is received can be identified by connecting the Receive Task to a Participant using a 
/// Message Flows within the definitional Collaboration of the Process
/// 
/// A Receive Task is often used to start a Process. In a sense, the Process is bootstrapped by the receipt of the Message. 
/// In order for the Receive Task to instantiate the Process its instantiate attribute MUST be set to true and it MUST NOT have any incoming Sequence Flow.
/// 
/// The Receive Task inherits the attributes and model associations of Activity. 
/// In addition the following constraints apply when the Receive Task references a Message: 
/// The Receive Task has at most one outputSet and at most one Data output. 
/// If the Data output is present, it MUST have an ItemDefinition equivalent to the one
...
/// </summary>
public class ReceiveTask(IFlowElementsContainer flowElementsContainer,
    string id, string name) : Task(flowElementsContainer, id, name, eActivityType.ReceiveTask), IMessageContainer
{
    /// <summary>
    ...
    /// One or more corresponding incoming Message Flows MAY be shown on the diagram.
    /// However, the display of the Message Flows is NOT REQUIRED. 
    /// The Message is applied to all incoming Message Flows, but can arrive for only one (1) of the 
    /// incoming Message Flows for a single instance of the Task.
    /// </summary>
    ...
    /// (i.e., there are no incoming Sequence Flows).
