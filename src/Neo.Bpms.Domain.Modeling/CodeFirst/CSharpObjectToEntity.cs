namespace Neo.Bpms.Domain.Modeling.CodeFirst;
...
public class CSharpObjectToEntity(INameSpaceRepository nameSpaceRepository) : CSharpObjectToModel
{
    ...
    private void GetEntityMetaFromAttributes(Entity entity, IEnumerable<object> attributes)
    {
        ...
        foreach (object attr in attributes)
        {
            ...
            switch (attr)
            {
                ...
                case EAttr_DefaultOrderBy _:
                    //TODO
                    break;
                case Entity_Index attrIndex:
                    {
                        EntityIndex idx = attrIndex.IsUnique
        ...
        if (ReflectionTools.IsInBaseInterface<IDomainEventEntity>(entity.EntityType))
        {
            entity.Auditable = ReflectionTools.IsInBaseInterface<IBaseAuditableEntity>(entity.EntityType) ? AuditableVersion.V2 : null;
        }
